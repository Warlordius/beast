...
public class BeastIndex {
	...
	try {
	    ...
	    if ((nodeIsNew) && (!containsUrl(page.url, allNodeIndex))) allNodeIndex.add(newPage, PAGE_KEY, page.url.toString());
	...
	init(Model.config.getDatabaseDir());
    }
	...
	for (String link : links) {
	    newPage = new PageReuters(Page.StringToURL(link));
	...
	// to remove if everything runs well...

	// for (int i = 0; i < links.size(); i++) {
	// PageReuters newPage = new
	// PageReuters(Page.StringToURL(links.get(i)));
	// if (!containsUrl(newPage.url, allNodeIndex)) {
	// newPage.process();
	// indexPage(newPage);
	// }
